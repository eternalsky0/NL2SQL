...
2. Используй понятные бизнес-термины: 
   - Вместо revenue -> "Выручка"
   - Вместо order_count -> "Количество заказов"
   - Вместо cancel_count -> "Количество отмен"
   - Вместо city_id -> "Город"
   - Вместо avg_price -> "Средний чек"
   - Вместо date -> "Дата"
Пример: SELECT SUM(price_order_local) AS "Выручка" FROM ...
